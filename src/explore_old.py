...
#        self.laser_sub = rospy.Subscriber('/front/scan', LaserScan, self.LidarCb)
...
#	    if calc_distance([x,y],[mx,my]) < 0.015 and abs(mw) < 0.01:
    ...
    def sendNavCb(self,data):
		...
		if calc_distance(self.currentpos,self.nextpnt) < 1 and calc_distance(self.scaledprevpnt,self.nextpnt) > 5:
...
#                        self.sendGoal(self.nextpnt)
		    ...
		    if nearestDist < 1:
			diff = np.asarray(nearestPt) - np.asarray([x, y])
    			ang = np.arctan2(diff[1],diff[0])
			if ang < 0:
			    ang = ang + math.pi
			else:
			    ang = ang - math.pi
			x = round(x + 1*math.cos(ang))
	     	        y = round(y + 1*math.sin(ang))
		    self.sendGoal([x, y])
		    self.nextpnt = [x, y]
